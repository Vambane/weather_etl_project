...
def transform_weather_data(
    ...
    longitude: float,
    city: str,
    dqc_enabled: bool = True,
    expected_hours: int | None = 168,
    ...
    if len(set(lengths.values())) != 1:
        raise ValueError(f"Hourly fields have inconsistent lengths: {lengths}")
    ...
    if dqc_enabled:
        if expected_hours is not None and not is_historical and len(df) != expected_hours:
        ...
        if not is_historical:
            ...
            if now_utc - latest_ts > pd.Timedelta(hours=6):
                raise ValueError(
